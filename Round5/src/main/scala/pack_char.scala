...
object pack_char {
  ...
  def pack_pk(sigma :Array[Byte] , B : Array[Array[Polynome_char]]) : BitString = {
    ...
    sigma foreach (x => {
      appendZeros(x.toBinaryString,8) foreach(c => result.+:(c.toString))
    })
  ...
  def unpack_pk(pk: BitString, sigma_size : Int,B_element : Int, B_element_bits : Int):(BitString,Array[Array[Polynome]])  ={
    ...
    (0 until b_nb_row).foreach( i => {
      (0 until m_bar).foreach(j => {
        val poly =  new Polynome(Array.ofDim[BitString](n),false,Char.MaxValue)
        (0 until n).foreach(k => {
  ...
  def unpack_ct(ct: BitString,u_elem : Int,u_size:Int , v_elem:Int,v_size:Int) = {
    println(ct.bits.length)
    println(u_elem * u_size + v_elem * v_size)
    val u_nb_rows = u_elem/m_bar/n
    val matrix = Array.ofDim[Polynome_char](m_bar,u_nb_rows)
    val v = Array.ofDim[BitString](v_elem)
    (0 until m_bar).foreach( i => {
      (0 until u_nb_rows).foreach(j => {
        val poly =  new Polynome_char(Array.ofDim[Char](n),false,Char.MaxValue)
        (0 until n).foreach( k => {
          poly.coef(k) = Integer.parseUnsignedInt(
            BitString.booleanToString(ct.bits.slice(i* m_bar* u_size * n  +  j*u_size * n  + k*u_size , i* m_bar * u_size * n + j*u_size * n + (k+1)*u_size).reverse)).toChar
            //(b_nb_row * nb_elem_per_row *nb_coeff_in_poly + where we at the row)
        })
        matrix(i)(j) = poly
      })
    ...
    (0 until v_elem).foreach(i => {
     ...
     bitstring.bits =  bitstring.bits ::: ct.bits.slice(u_elem*u_size + i * v_size,u_elem*u_size + (i+1)* v_size)
      v(i) = bitstring
    })
